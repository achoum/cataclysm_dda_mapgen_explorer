...
public class Tiles {
    ...
    public String tilesetPath(String main_directory, String tileset) {
        return Paths.get(main_directory, "gfx", tileset).toString();
    }

    public void load(String main_directory, String tileset) throws Exception {
        load(main_directory, new String[]{tileset});
    }

    public void load(String main_directory, String[] tilesets) throws Exception {
        assert tilesets.length > 0;
        if (tilesets[0].equals("ASCII")) {
        ...
        // Find the first valid tileset.
        int valid_tileset_idx = 0;
        while (valid_tileset_idx < tilesets.length) {
            if (new File(tilesetPath(main_directory, tilesets[valid_tileset_idx])).exists()) {
                break;
            }
            valid_tileset_idx++;
        }
        if (valid_tileset_idx == tilesets.length) {
            // No tileset found. Use ASCII tileset.
            Resources.tiles.force_character = true;
            return;
        }

        String tile_directory =
                tilesetPath(main_directory, tilesets[valid_tileset_idx]);
